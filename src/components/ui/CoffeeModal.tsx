...
import { X, Sparkles, Coffee, Copy, Check, Heart, ShieldCheck, QrCode } from 'lucide-react';
...
export const CoffeeModal: React.FC<CoffeeModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-sm sm:max-w-md my-auto rounded-[36px] overflow-hidden bg-gradient-to-b from-[#FFF5F8] via-[#FFEBF1] to-[#FFF0F4] dark:from-[#1E1217] dark:via-[#26151E] dark:to-[#180E13] border border-pink-200/60 dark:border-pink-500/30 shadow-[0_25px_70px_rgba(255,50,100,0.3)] text-gray-800 dark:text-gray-100 transition-all">
        
        {/* Decorative background sparkles & floating blurs */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 bg-pink-400/25 dark:bg-pink-500/20 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-0 right-0 w-48 h-48 bg-amber-300/25 dark:bg-amber-500/15 rounded-full blur-2xl pointer-events-none"></div>
        
        {/* Floating Sparkle Icons */}
        <Sparkles className="absolute top-6 left-6 w-4 h-4 text-pink-400 animate-pulse pointer-events-none" />
        <Sparkles className="absolute top-12 right-12 w-3 h-3 text-amber-400 animate-ping pointer-events-none" />
        <Coffee className="absolute bottom-6 right-6 w-5 h-5 text-pink-400/30 dark:text-pink-400/20 rotate-12 pointer-events-none" />

        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 w-9 h-9 rounded-full bg-white/80 dark:bg-black/50 hover:bg-white dark:hover:bg-black/80 border border-pink-200 dark:border-white/10 flex items-center justify-center text-gray-500 dark:text-gray-300 hover:text-pink-600 transition-all shadow-md"
          title="Đóng modal"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="relative z-10 p-5 sm:p-7 space-y-4 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-600 dark:text-gray-300 max-w-xs mx-auto leading-relaxed pt-0.5 font-medium">
          ...
          <div className="bg-white dark:bg-[#150D11] border border-pink-100 dark:border-pink-500/20 rounded-3xl p-4 sm:p-5 shadow-xl space-y-3.5">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-300 font-bold flex items-center justify-center gap-1.5">
              <QrCode className="w-3.5 h-3.5 text-pink-500" />
              <span>Quét mã QR để mời NGUYENMMO một ly cafe ☕</span>
            </div>

            {/* Clean QR Code Display */}
            <div className="relative mx-auto w-full max-w-[240px] sm:max-w-[260px] rounded-2xl overflow-hidden border-2 border-pink-200/80 dark:border-pink-500/40 bg-white p-2.5 shadow-md">
              <img
            ...
            <div className="bg-pink-50/80 dark:bg-pink-950/40 border border-pink-100 dark:border-pink-900/50 rounded-2xl p-3 text-left space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-gray-500 dark:text-gray-400 font-medium">Ngân hàng:</span>
                <div className="flex items-center gap-1.5">
                  <span className="px-2 py-0.5 bg-red-500 text-white font-black rounded text-[10px] uppercase font-mono">MBV</span>
                  <span className="font-bold text-pink-600 dark:text-pink-400 font-mono">MB Bank</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-1.5 border-t border-pink-100 dark:border-pink-900/40">
