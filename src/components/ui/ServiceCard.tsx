...
import { Star, Heart, ShoppingCart, Zap, CheckCircle2, ShieldCheck } from 'lucide-react';
...
export const ServiceCard: React.FC<ServiceCardProps> = ({ service }) => {
  ...
  const handleQuickAdd = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    addToCart(service, 'https://example.com/link-demo', service.min || 1);
  ...
  return (
    <div className="group relative border-beam-card p-5 flex flex-col justify-between transition-all duration-300">
      ...
      <div>
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 rounded-xl bg-neon-red/10 border border-neon-red/30 flex items-center justify-center text-xl shadow-inner">
              {service.icon || '🚀'}
            </div>
            <div>
              <span className="text-[10px] font-bold tracking-wider px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-gray-300 uppercase">
                {service.category}
              </span>
      ...
      <div>
        {/* Rating & Sold */}
        <div className="flex items-center justify-between text-xs text-gray-400 pb-3 mb-3 border-b border-white/5 font-mono">
        ...
        <div className="flex items-baseline justify-between mb-4">
          ...
          {service.vipPrice && service.vipPrice < displayPrice && (
            <div className="text-right">
              ...
              <div className="text-xs font-bold text-gold-300 font-mono">
        ...
        <div className="grid grid-cols-5 gap-2">
          <Link
            ...
            className="col-span-4 py-2.5 px-3 bg-neon-red hover:bg-neon-red-hover text-white text-xs font-bold rounded-xl btn-beam-touch text-center hover:scale-[1.02] transition-all flex items-center justify-center gap-1.5 overflow-hidden"
          >
            <span>ĐẶT DỊCH VỤ</span>
          </Link>
          <button
            onClick={handleQuickAdd}
            className="col-span-1 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 hover:text-white rounded-xl flex items-center justify-center transition-all"
