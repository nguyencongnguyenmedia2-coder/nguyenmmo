...
export const ChatWidget: React.FC = () => {
  ...
  return (
    <>
      {/* 🚀 SINGLE UNIFIED FLOATING CONTAINER FOR ALL 3 BUTTONS & MODALS */}
      <div className={`fixed ${mobileBottomClass} md:bottom-6 right-3 sm:right-6 z-40 flex flex-col items-end gap-2.5 transition-all duration-300`}>
        ...
        {showTooltip && !isSupportOpen && !isAiChatOpen && (
        ...
        {/* 🤖 1. AI CHATBOT WINDOW MODAL */}
        {isAiChatOpen && (
          <div className="mb-2 w-[320px] sm:w-[380px] h-[480px] sm:h-[500px] bg-[#0C0C14]/95 backdrop-blur-2xl border border-purple-500/30 rounded-[28px] shadow-[0_20px_60px_rgba(150,50,255,0.25)] text-white flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5">
        ...
        >
          <Bot className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-white animate-pulse shrink-0" />
        ...
        <button
          ...
          className="group relative p-3 sm:px-4 sm:py-2.5 rounded-full bg-neon-red hover:bg-neon-red-hover text-white font-bold text-xs sm:text-sm shadow-neon-red flex items-center gap-2 hover:scale-105 active:scale-95 transition-all border border-white/20"
          aria-label="Hỗ trợ khách hàng"
        >
          <MessageCircle className="w-4.5 h-4.5 sm:w-5 sm:h-5 fill-white animate-pulse shrink-0" />
          <span className="hidden sm:inline font-black tracking-wide">💬 Hỗ trợ</span>
          <span className="sm:hidden text-xs font-black pr-0.5">Hỗ trợ</span>
        </button>
