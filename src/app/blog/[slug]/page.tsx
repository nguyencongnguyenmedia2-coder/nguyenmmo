  ...
  const tocItems = useMemo(() => {
    ...
    lines.forEach((line, index) => {
      ...
      if (
        ...
        trimmed.startsWith('🤖') ||
        trimmed.startsWith('🎓 LỘ TRÌNH') ||
        trimmed.startsWith('💰 BƯỚC') ||
        trimmed.startsWith('🚀 TRỤ CỘT') ||
        trimmed.startsWith('🎵')
      ) {
        headings.push({
          text: trimmed,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-gray-400 font-mono">
        ...
        <Link
          href="/blog"
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold text-gray-300 hover:text-white transition-all"
        >
      ...
      {/* 🖼️ FEATURED BANNER IMAGE (Optimal 16:9 display) */}
